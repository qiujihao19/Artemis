...
from artemis.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, MAX_IMAGE_LENGTH, \
    MAX_VIDEO_LENGTH, IN_THE_VIDEO_QUESTION, TRACKING_LIST_NOTION, BOX_TOKEN_INDEX
from torch.utils.data import Dataset
from artemis.train.llava_trainer import LLaVATrainer

from artemis import conversation as conversation_lib
from artemis.model import *
from artemis.mm_utils import tokenizer_image_token

from PIL import Image
from artemis.utils import order_pick_k
import pickle
local_rank = None
...
class ModelArguments:
    ...
    mm_use_bbox_token: bool = field(default=False)
    # ===================================================================
    image_tower: Optional[str] = field(default=None)
    # ===================================================================
    spi_model: bool = field(default=False)
    ...
    num_level_spi_features: Optional[int] = field(default=4)
    # ===================================================================
    k_means: bool = field(default=False)
    kmeans_n_clusters: Optional[int] = field(default=4)
    kmeans_max_iter: Optional[int] = field(default=3)
    # ===================================================================
...
class DataArguments:
    ...
    num_inputbox: int = 4
    num_trackbox: int = 8
    choosen_mode: Optional[str] = field(default='average')
    train_mode: Optional[str] = field(default='noreferring')
    # ===================================================================
@dataclass
...
def get_conv(caption, question, num_inputbox):
    if question is None:
        question = random.sample(IN_THE_VIDEO_QUESTION, 1)[0]
    assert '<bbox>' in question
    if num_inputbox > 0:    
        track_note = random.sample(TRACKING_LIST_NOTION, 1)[0]
        question += ' ' + track_note
        question += '<bbox>' * num_inputbox + '.'
    question = '<video>\n' + question  # put <image> in the most front     
    conversations = [
    {
        "from": "human",
        "value": question
...
class LazyReferringSupervisedDataset(Dataset):
    """Dataset for supervised video referring fine-tuning."""

    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(LazyReferringSupervisedDataset, self).__init__()
        # ================================================
        list_data_dict = []
    ...
    def modality_lengths(self):
        ...
        return length_list    
    def avg(self, len_list, num_box):
        if num_box <= 0:
            return []
        interval = len(len_list) // num_box
        # random select the start frame
        start = random.choice(list(range(interval)))
        # add noise to next frames
        noise = interval / 4 #noise,note:/(>2)
        dn_point = start
        choosen_list = [len_list[dn_point]]
        for _ in range(num_box - 1):
            dn_point = dn_point + interval
            noise_n = int(noise * ((random.random() - 0.5) * 2))
            pos = min(max(dn_point + noise_n, 0), len(len_list) - 1)
            choosen_list.append(len_list[pos])
        return choosen_list
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            # ======================================================================================================
            video_file = self.list_data_dict[i]['clip_path']
            video_folder = self.data_args.video_folder
            image_processor = self.data_args.image_processor
            video = os.path.join(video_folder, video_file)
            # the name image is to remain the same as video llava
            with open(video, 'rb') as f:
                image = pickle.load(f)
            image = [torch.tensor(image)] 
            
            caption = self.list_data_dict[i]['caption']
            question = self.list_data_dict[i].get('question', None)
            conversations = get_conv(caption, question, self.data_args.num_inputbox)
            ...
            assert (data_dict['input_ids'] == BOX_TOKEN_INDEX).sum() == self.data_args.num_inputbox + 1
            # ==========================================================================================================
            ...
            if self.list_data_dict[i]['key_frame_box'] and self.list_data_dict[i]['key_frame_id']:
                box_list.append(self.list_data_dict[i]['key_frame_box'])
                file = self.list_data_dict[i]['key_frame_id']
                img_list.append(Image.open(os.path.join(video_folder, frame_path, file)).convert('RGB'))
            else:
            ...
            for cnt in img_idx_list:
                ...
                img_path = os.path.join(video_folder, frame_path, img_name)
                img_list.append(Image.open(img_path).convert('RGB'))
                box_list.append(tracklist[cnt])
                    
            assert len(img_list) == len(box_list) == self.data_args.num_trackbox + 1
            images_clip = [image_processor.preprocess(i, return_tensors='pt')['pixel_values'][0] for i in img_list]
            
            w, h = img_list[0].size
            bboxes = [torch.tensor(box) / torch.tensor([w, h, w, h], dtype=torch.half) for box in box_list]
            ...
            data_dict['images4box'] = images_clip
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0] and 'video' not in sources[0]:
                ...
                # rank0_print('video')
                video_file = self.list_data_dict[i]['video_clip']
                video_folder = self.data_args.video_folder
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'images4box' in instances[0]:
            images4box = [instance['images4box'] for instance in instances]

            # adapt to multi-video or multi-image or multi-image & video
            new_images4box = []
            for image in images4box:
                if type(image) is list:
                    for i in image:
                        new_images4box.append(i)
                else:
                    new_images4box.append(image)
            images4box = new_images4box

        # ==========Too many videos or images may lead to OOM, so we encode them one by one======================
            batch['images4box'] = images4box
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if data_args.train_mode == 'referring':
        train_dataset = LazyReferringSupervisedDataset(tokenizer=tokenizer,
...
def train():
    ...
    if model_args.image_tower is not None:
        ...
        if model_args.spi_model:
            # the spi model can only support fp16
            assert training_args.bf16 == False
            spi_model = model.get_spi_model()
            spi_model.to(dtype=torch.float16, device=training_args.device)
    ...
    # =============================================================================================================
        ...
        if model_args.spi_model:
            for p in model.get_spi_model().parameters():
                p.requires_grad = True
        ...
        model.config.mm_use_bbox_token = model_args.mm_use_bbox_token
